 ...
 * File:   main.cpp
 * Author: Daniel Baca
 * Date: September 5, 2022, 10:20 AM
 * Purpose: Showing the growing population based on user input values.
 */
...
int main(int argc, char** argv) 
{
    ...
    cout << "What is the average daily population increase?(Ex. 0.05%, 0.07%) ";
     cin >> perc_incr;
    cout << endl;
    ...
    // Even more input validation
     if (num_days < 1)
     {
        cout << "Please Enter a number of days higher than or equal to 1.";
     }
     else
     {
     ...
     while (count <= num_days)
     {
        if (count == 1)
        {
            cout << "Day " << count << ": " << fixed << showpoint 
                 << setprecision(2) << population << endl;
